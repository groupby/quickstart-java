...
<%@include file="includes/form.jsp"%>
  ...
  <div class="container">
    <c:choose>
      ...
      <c:otherwise>
        <div id="ruleFired">
          <div id="ruleTitle">Rule fired: ${results.template.ruleName} (Template: ${results.template.name}) <a href="javascript:;" onclick="showZones()" class="btn sml">show zones</a></div>
          <div id="allZones" style="display:${cookie.showZones.value ? 'block' : 'none'}">
            <c:if test="${fn:length(results.template.zones) == 0}">
              <div class="zoneResponse">No zones</div>
            </c:if>
            <c:if test="${fn:length(results.template.zones) > 0}">
              <div class="zoneResponse">
