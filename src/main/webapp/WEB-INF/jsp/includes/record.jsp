<c:forEach items="${results.records}" var="record" varStatus="i">
    ...
    <li class="record highlightCorresponding h${record.id}" data-id="row${b.index}.h${record.id}">
        ...
        <div class="details">
            ...
            <c:if test="${!empty record.allMeta['injectedImage']}">
              <div class="keyValue"><span class="key">${cookie.imageField.value}</span>:
              <span class="value"><c:out value="${record.allMeta['injectedImage']}"/></span></div>
              <img style="display: inline-block; max-width: 200px; max-height: 200px; margin: 5px;" src="${URLDecoder.decode(cookie.imagePrefix.value)}<c:out value="${record.allMeta['injectedImage']}"/>${URLDecoder.decode(cookie.imageSuffix.value)}"/>
            </c:if>
        </div>
    </li>
</c:forEach>
